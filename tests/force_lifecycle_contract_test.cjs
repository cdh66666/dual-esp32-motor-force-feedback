...
async function scenario(failStatus = false, failStop = false, largeOffset = false) {
  const commands = [];
  const pair = [
  ...
  const context = vm.createContext({
    ...
    forcePair:async()=>pair, ensureReady:async()=>{},clearMotionTimers:()=>{},
    ...
    send:async(port,command)=>{
      ...
      return {acknowledged:true,reply:command==='sync status' ?
        `SYNC mode=force armed=${failStatus && port==='B' ? 0 : 1} timeout=10` : command==='model' ? 'MODEL fw=0.5.8-coast-autotune' : 'OK'};
    },
  });
  ...
  if(largeOffset)assert.equal(commands.filter(c=>c.command==='encreset').length,2);
...
(async()=>{
  ...
  console.log('PASS: force startup, paired rollback, torque normalization, offsets, stop acknowledgment');
